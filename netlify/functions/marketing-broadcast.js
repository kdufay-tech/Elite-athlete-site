...
export default async(req)=>{
  ...
  if(audience==='invited'||audience==='all'){
    let page=0;
    while(true){
      const r=await fetch(`${supabaseUrl}/rest/v1/beta_invites?status=in.(sent,pending)&select=email&limit=1000&offset=${page*1000}`,{headers:{apikey:serviceKey,Authorization:`Bearer ${serviceKey}`}});
  ...
  const audienceId=await getOrCreateAudience(resendKey);
  ...
  if(!sRes.ok)return new Response(JSON.stringify({error:`Send failed: ${sData.message}`}),{status:500,headers:CORS});
  return new Response(JSON.stringify({ok:true,message:`Broadcast launched to ${emails.length.toLocaleString()} recipients`,total:emails.length,added,contactsFailed:cFailed,broadcastId:bData.id}),{status:200,headers:CORS});
};
